...
_xgem_cache_lock = threading.Lock()
_xgem_model_cache: dict = {"key": None, "bundle": None}
...
def _generate_ct_volume_with_text2ct(payload: dict, output_dir: Path) -> tuple[np.ndarray, tuple[float, float, float]]:
    ...
    if "text2ctDim" in payload:
        ...
        # Use a smaller default target volume on GPU unless explicitly overridden.
        args.diffusion_unet_inference["dim"] = [256, 256, 64]
...
def _resolve_xgem_root(payload: dict) -> Path:
    ...
    root = _resolve_existing_path([c for c in candidates if c is not None])
    if root is None:
        raise RuntimeError(
            "XGeM_API root not found. Set payload.xgemApiRoot or env XGEM_API_ROOT."
        )
    return root


@contextmanager
def _temporary_env(vars_to_set: dict):
    old_values = {}
    try:
...
def _load_xgem_modules(xgem_root: Path):
    if str(xgem_root) not in sys.path:
        sys.path.insert(0, str(xgem_root))
    with _pushd(xgem_root):
        model_loader_mod = importlib.import_module("inference.model_loader")
        tasks_mod = importlib.import_module("inference.tasks")
    return model_loader_mod.load_model, tasks_mod.run_inference_task


def _get_xgem_model_bundle(payload: dict) -> dict:
    ...
    cache_key = (
        str(xgem_root.resolve()),
        xgem_device or "",
        xgem_model_path or "",
        xgem_weights_dir or "",
    )

    with _xgem_cache_lock:
        cached = _xgem_model_cache.get("bundle")
        if _xgem_model_cache.get("key") != cache_key or cached is None:
            _set_progress("XGeM: loading model")
            load_model, run_inference_task = _load_xgem_modules(xgem_root)
            with _temporary_env(
                {
                    "XGEM_MODEL_PATH": xgem_model_path,
                    "XGEM_WEIGHTS_DIR": xgem_weights_dir,
                }
            ):
                model = load_model(device=xgem_device)
            cached = {
                "root": xgem_root,
                "model": model,
                "run_inference_task": run_inference_task,
            }
            _xgem_model_cache["key"] = cache_key
            _xgem_model_cache["bundle"] = cached
    return cached
    ...
    if proc.returncode != 0:
        ...
        if not detail:
            detail = (
                ...
                f"Inspect logs: {stdout_log.name}, {stderr_log.name}"
            )
...
def _load_xray_pixels_for_generation(payload: dict, output_dir: Path) -> np.ndarray:
    source = (payload.get("xraySource") or "xgem").lower()
    if source == "asset":
        payload["_xrayEffectiveSource"] = "asset"
        return _load_xray_pixels()
    payload["_xrayEffectiveSource"] = "xgem"
    return _generate_xray_pixels_with_xgem(payload, output_dir)
...
def _write_xray_dicom(
    ...
    pixels = _load_xray_pixels_for_generation(payload, output_dir)
...
def _simulate_generation_job(file_id: str, payload: dict, series_instance_uid: str) -> None:
    ...
    try:
        ...
        xray_mode = payload.get("_xrayEffectiveSource", "n/a") if generation_type == "xray" else "n/a"
