...
from .clip_modules.modeling_clip import CLIP3DModel
...
class FrozenCLIP(AbstractEncoder):
    def __init__(self,
        ...
        if max_length != 77:
            config.text_config.max_length = max_length
    ...
    def forward(self, input, encode_type):
        if encode_type == 'encode_text':
            ...
            if input.shape[1] == 1 and len(input.shape) == 4:
                input = torch.cat([input, input, input], dim=1)
            return self.encode_vision(input)


@register('clip_3D', version)
class FrozenCLIP3D(AbstractEncoder):
    def __init__(self,
                 version="openai/clip-vit-large-patch14",
                 max_length=77,
                 encode_type='encode_text',
        ...
        if vision_embed_dim is None:
            vision_embed_dim = 786432
        
        self.model = CLIP3DModel(config, vision_encoder, vision_embed_dim)
        self.model.cuda()
    ...
    def encode_vision_noproj(self, vision_inputs):
        outputs = self.model.vision_model(vision_inputs, return_encoded_tokens=True)
        return outputs
    ...
    def encode_vision(self, images):
        z = self.encode_vision_noproj(images)
        h_r, w_r, z_r = z.shape[1], z.shape[2], z.shape[3]
        z = torch.mean(z, dim=1)
        z = z.view(z.shape[0], -1)
        image_embeds = z[:, :] if z.ndim == 3 else z
        
        image_latents = self.model.visual_projection(image_embeds)
        image_latents = image_latents / torch.norm(image_latents, dim=-1, keepdim=True)
        return image_latents.unsqueeze(1)
