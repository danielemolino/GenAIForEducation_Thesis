...
class MaskGITTransformer(nn.Module):
    ...
    def sample(
        ...
        *,
        ...
        if has_prime:
            with torch.no_grad():
                prime_token_ids = self.ctvit(prime_frames, return_only_codebook_ids = True)
        ...
        num_tokens = self.ctvit.num_tokens_per_frames(num_frames, include_first_frame = not exists(prime_frames))
        ...
        patch_shape = self.ctvit.get_video_patch_shape(num_frames + prime_num_frames, include_first_frame = True)
    ...
    def forward(
        ...
        *,
        ...
        assert not (exists(videos) and not exists(self.ctvit)), 'ctvit must be provided if one wants to encode the videos live during training'
...
def make_video(
    ...
    for text, scene_num_frames, next_scene_prime_length in zip(texts, num_frames, prime_lengths):
        video = maskgittransformer.sample(texts = text, prime_frames = video_prime, num_frames = scene_num_frames)
        scenes.append(video)
