...
export default function ModeRoute({
  ...
  const allowEmptyStudies = !!route?.allowEmptyStudies;
  ...
  useEffect(() => {
    if (!ExtensionDependenciesLoaded || (!studyInstanceUIDs?.length && !allowEmptyStudies)) {
      return;
    }
    ...
    if ((Array.isArray(studyInstanceUIDs) && studyInstanceUIDs[0]) || allowEmptyStudies) {
      retrieveLayoutData();
    }
    return () => {
      layoutTemplateData.current = null;
    };
  }, [studyInstanceUIDs, ExtensionDependenciesLoaded, allowEmptyStudies]);

  useEffect(() => {
    if (!hotkeys || !ExtensionDependenciesLoaded || (!studyInstanceUIDs?.length && !allowEmptyStudies)) {
      return;
    }
  ...
  }, [ExtensionDependenciesLoaded, hotkeys, studyInstanceUIDs, allowEmptyStudies]);

  useEffect(() => {
    if (
      !layoutTemplateData.current ||
      !ExtensionDependenciesLoaded ||
      (!studyInstanceUIDs?.length && !allowEmptyStudies)
    ) {
      return;
    }
  ...
  if ((!studyInstanceUIDs && !allowEmptyStudies) || !layoutTemplateData.current || !ExtensionDependenciesLoaded) {
    return null;
  }
