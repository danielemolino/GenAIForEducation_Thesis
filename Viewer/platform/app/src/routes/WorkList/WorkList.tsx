...
function WorkList({
  ...
  const tableDataSource = sortedStudies.map((study, key) => {
    ...
    const visibleModesForStudy = getVisibleModesForStudy({
      loadedModes: appConfig.loadedModes,
      groupEnabledModesFirst: appConfig.groupEnabledModesFirst,
      modalities,
      study,
    });
      ...
      expandedContent: (
        ...
        >
          <div className="flex flex-row gap-2">
            {visibleModesForStudy.map((mode, i) => {
...
function getVisibleModesForStudy({ loadedModes = [], groupEnabledModesFirst, modalities, study }) {
  const modalitiesList = getNormalizedModalities(modalities);
  const isCTStudy = modalitiesList.includes('CT');
  const isXrayStudy = modalitiesList.some(modality => XRAY_MODALITIES.has(modality));

  let visibleModes = [...loadedModes];
  if (isCTStudy) {
    visibleModes = visibleModes.filter(mode => CT_ALLOWED_ROUTE_NAMES.has(mode.routeName));
  } else if (isXrayStudy) {
    visibleModes = visibleModes.filter(mode => XRAY_ALLOWED_ROUTE_NAMES.has(mode.routeName));
  }

  if (groupEnabledModesFirst) {
    const modalitiesToCheck = (modalities || '').replaceAll('/', '\\');
    visibleModes.sort((a, b) => {
...
function getNormalizedModalities(modalities = '') {
  return modalities
    .replaceAll('\\', '/')
    .split('/')
